...
/**
 * Generate a Certificate of Indigency PDF.
 */
export async function generateIndigencyPdf({ request, barangay, officials, amount }) {
  ...
  doc.setFontSize(16);
  doc.setFont('helvetica', 'bold');
  doc.text('CERTIFICATE OF INDIGENCY', headerCenterX, y, { align: 'center' });
  y += 10;
  ...
  const bodySegments = [
    ...
    { text: ', ', bold: false },
    { text: `${ageNum} year old,`, bold: true },
    { text: `of legal age, Filipino Citizen, is a bonafide resident of ${locationStr}.`, bold: false },
  ];
  y = drawMixedParagraph(doc, bodySegments, contentLeft, y, bodyWidth - 8, 4.5, 10);
  y += 8;

  // ── Body paragraph 2: Indigent family ──
  doc.setFontSize(10);
  const indigentSegments = [
    { text: 'I further certify', bold: true },
    { text: ' that the name stated above belongs to an indigent family.', bold: false },
  ];
  y = drawMixedParagraph(doc, indigentSegments, contentLeft, y, bodyWidth - 8, 4.5, 10);
  y += 8;

  // ── Body paragraph 3: Purpose ──
  doc.setFontSize(10);
  const purposeSegments = [
    { text: 'This certification is being issued upon request of the interested person for ', bold: false },
    { text: 'reference and record purposes', bold: true },
    { text: '.', bold: false },
  ];
